...
function generalRate(grams) {
    ...
    if (grams >= 100 && grams <= 199) {
        return "✉ 33 กล่อง AA-A = 38 // AB-B = 38";
    }
    if (grams >= 200 && grams <= 299) {
        return "✉ 33 กล่อง AA-A = 38 // AB-B = 38";
    }
    ...
    if (grams >= 1000 && grams <= 1499) {
        return    "✉ 48 ถึง 53 กล่อง C-D = 68-69 "; 
    }
    if (grams >= 1500 && grams <= 1999) {
        return "✉ 68 ถึง 73 กล่อง E-Fเล็ก = 78-79";
    }
    if (grams >= 2000 && grams <= 2499) {
        return "✉ 63 ถึง 73 กล่อง F-G = 78-79";
    }
    if (grams >= 2500 && grams <= 2999) {
...
function calculatePrice() {
    ...
    if (grams === null || grams <= 0) {
        resultElement.innerHTML = '<strong>กรุณากรอกข้อมูลให้ถูกต้อง</strong>กรอกน้ำหนักเป็น kg หรือ g เช่น 1kg หรือ 500g';
    ...
    if (price !== null) {
        resultElement.innerHTML = `<strong>${customerName}</strong>${formatPrice(price)}<div class="result-detail">น้ำหนักที่คำนวณ: ${grams} กรัม</div>`;
        resultElement.classList.add('visible');
        resultElement.classList.remove('hidden');
    } else {
